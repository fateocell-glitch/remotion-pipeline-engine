...
export const ValueVerdict: React.FC<LayoutEffectProps> = ({cue, props}) => {
  ...
  return <div style={{position:"absolute",left:86,top:300,width:hasMetric?760:620,padding:"26px 30px 24px",borderRadius:14,color:"#FFFFFF",background:"rgba(5,12,21,.76)",border:"1px solid rgba(10,132,255,.68)",boxShadow:"0 12px 34px rgba(0,0,0,.32), 0 0 26px rgba(10,132,255,.18)",opacity,transform:"translateX("+translateX+"px)"}}>
    <div style={{color:"var(--primary-accent)",fontSize:17,fontWeight:950,letterSpacing:4}}>VALUE VERDICT</div>
    ...
    {hasMetric ? <div style={{position:"absolute",right:24,top:24,minWidth:210,padding:"16px 18px",borderRadius:12,background:"rgba(10,132,255,.14)",border:"1px solid rgba(10,132,255,.56)",textAlign:"right",opacity:interpolate(frame,[start+12,start+28],[0,1],{extrapolateLeft:"clamp",extrapolateRight:"clamp"}),transform:"scale("+interpolate(frame,[start+12,start+28],[.9,1],{extrapolateLeft:"clamp",extrapolateRight:"clamp"})+")"}}><div style={{fontSize:14,fontWeight:950,letterSpacing:2,color:"var(--primary-accent)"}}>{label}</div><div style={{marginTop:2,fontSize:52,lineHeight:"58px",fontWeight:950}}>{value}</div>{unit ? <div style={{fontSize:21,fontWeight:900,color:"rgba(255,255,255,.8)"}}>{unit}</div> : null}</div> : null}
  </div>;
};
