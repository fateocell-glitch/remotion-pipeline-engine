...
const normalizeLayer = (cue: LayeredCue, layer: EffectLayer | undefined, index: number): EffectLayer => {
  ...
  return {
    layerId: clean(current.layerId, "layer-" + (index + 1)),
    layout: current.layout ?? cue.layout,
    category: clean(current.category, clean(legacy.category, clean(legacy.eyebrow, clean(legacy.categoryTag, cue.section?.eyebrow ?? "DESIGN SYSTEM")))),
    headline: clean(current.headline, clean(legacy.headline, clean(legacy.title, cue.section?.subtitle ?? "核心观点"))),
    effectText: clean(current.effectText, clean(legacy.effectText, clean(legacy.effectZh, clean(legacy.body, cue.caption?.zh ?? "")))),
    ...(role ? {textRole: role, role} : {}),
    accent: semanticAccent,
    payload: {...payload, ...(role ? {role} : {}), accent: semanticAccent},
    commonProps: normalizeCommonProps(current.commonProps, current.enterOffset),
    enterOffset: normalizedOffset(current.commonProps?.enterOffset ?? current.enterOffset),
  };
};
