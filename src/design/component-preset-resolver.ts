...
export const resolveComponentProps = (layout: JasonWuCue["layout"], effectProps: Record<string, unknown> | undefined): Record<string, unknown> => ({...(effectProps ?? {}), designTokens:{...getComponentTokens(layout), ...(isTokenObject(effectProps?.designTokens) ? effectProps.designTokens : {})}, designPresetVersion:getComponentPreset(layout)?.version ?? 0});
export const resolveMotionWithPreset = (layout: JasonWuCue["layout"], commonProps: Partial<BaseLayerCommonProps> | undefined): Partial<BaseLayerCommonProps> => {
  const tokens = getComponentTokens(layout);
  const source = commonProps ?? {};
  return {...source, position: source.position && source.position !== "center" ? source.position : tokens.position, scale: typeof source.scale === "number" && source.scale !== 1 ? source.scale : tokens.scale, enterAnimation: source.enterAnimation && source.enterAnimation !== "spring-up" ? source.enterAnimation : tokens.spring, sfx: source.sfx && source.sfx !== "none" ? source.sfx : tokens.sfx};
};
export const resolveFaceAwareLayerForRender = (layout: JasonWuCue["layout"], commonProps: Partial<BaseLayerCommonProps> | undefined, faceZone?: FaceZone | null, beatIndex = 0, sceneMode?: "speaker_mode" | "cinematic_mode") => {
  const sourceTokens = getComponentTokens(layout);
  const sourcePreset = getComponentPreset(layout);
  const resolved = resolveFaceAwareLayer({layout, commonProps: resolveMotionWithPreset(layout, commonProps), tokens: sourceTokens, faceZone, family: sourcePreset?.family, displayIntent: sourcePreset?.displayIntent ?? "side-overlay", candidates: registry.components, beatIndex, sceneMode});
