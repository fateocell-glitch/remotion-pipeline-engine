...
const layouts = componentAssetRegistry.components.map((component) => component.id);
...
createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/admin/component-weights") {if(!isLocalAdmin(req)) return send(res,403,"Local super-admin access required.","text/plain"); return send(res,200,buildComponentWeightsPage(),"text/html; charset=utf-8");}
    if (req.method === "GET" && url.pathname === "/component-weights.js") {if(!isLocalAdmin(req)) return send(res,403,"Local super-admin access required.","text/plain"); const file=join(root,"public","component-weights.js");if(!existsSync(file))return send(res,404,"Component weights bundle missing","text/plain");return send(res,200,await readFile(file),"application/javascript; charset=utf-8");}
