...
const base = {layout:"engineering-return",commonProps:{position:"center",offsetX:0,offsetY:0,scale:1},tokens:{mountMode:"center",mountX:0,mountY:0,boundsX:110,boundsY:210,boundsWidth:1580,boundsHeight:456,scale:1},family:"metrics",candidates:[]};
...
test("right-side face moves a colliding component to the left", () => {
  ...
  assert.equal(result.avoidance.reason, "face-right");
});

test("large center close-up scales down and moves below the face", () => {
  ...
  assert.equal(result.tokens.mountMode, "bottom");
  assert.equal(result.commonProps.scale, .8);
  assert.equal(result.avoidance.reason, "large-center-face");
});
