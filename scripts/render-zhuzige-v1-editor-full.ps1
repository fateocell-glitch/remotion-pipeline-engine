...
$OutDir = Join-Path $Root "out/segments-v1-editor-full"
$Final = Join-Path $Root "out/zhuzigeceo-remake-v1-editor-full.mp4"
$ConcatList = Join-Path $OutDir "zhuzige-editor-full-concat.txt"
...
foreach ($Segment in $Segments) {
  $file = Join-Path $OutDir ("zhuzige-editor-" + $Segment.Name + ".mp4")
  ...
  for ($attempt = 1; $attempt -le 3; $attempt++) {
    & $Remotion render src/index.ts ZhuzigeEditorDraft $file --frames=$($Segment.Frames) --codec=h264 --crf=20 --pixel-format=yuv420p --concurrency=4 --x264-preset=veryfast
    if ($LASTEXITCODE -eq 0) { $succeeded = $true; break }
    Start-Sleep -Seconds 8
  }
  if (-not $succeeded) { throw "Beat render failed after 3 attempts: $($Segment.Name)" }
}$ConcatLines = $Segments | ForEach-Object { "file '$((Join-Path $OutDir ('zhuzige-editor-' + $_.Name + '.mp4')).Replace('\', '/'))'" }
